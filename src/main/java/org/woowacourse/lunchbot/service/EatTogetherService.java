...
public class EatTogetherService {
    ...
    public static final Function<LocalTime, Boolean> applyTime;
    public static final Function<LocalTime, Boolean> resultTime;
    ...
    static {
        applyTime = time -> (time.compareTo(LocalTime.of(9, 0)) >= 0
                && time.compareTo(LocalTime.of(11, 20)) < 0)
                || (time.compareTo(LocalTime.of(16, 0)) >= 0
                && time.compareTo(LocalTime.of(17, 30)) < 0);

        resultTime = now -> (now.compareTo(LocalTime.of(11, 30)) >= 0
                && now.compareTo(LocalTime.of(13, 30)) < 0)
                || (now.compareTo(LocalTime.of(18, 0)) >= 0
                && now.compareTo(LocalTime.of(20, 0)) < 0);
    }

    static public boolean aa() {
